...
typedef uint32_t u32_may_alias __attribute__((may_alias));

static inline uint32_t ct_select_u32(uint32_t oldv, uint32_t newv,
                                     uint32_t mask) {
  return (mask & newv) | (~mask & oldv);
}
...
int64_t bucket_chaining_join(const struct table_t *const R,
  ...
  for (uint32_t i = 0; i < numS; i++) {
    ...
    for (int hit = bucket[idx]; hit > 0; hit = next[hit - 1]) {
      ...
      const uint32_t m = (uint32_t)match;
      if (!isSPrimary) {
        const uint32_t src =
            *(const u32_may_alias *)(const void *)Rtuples[hit - 1].paySelf;
        u32_may_alias *dst = (u32_may_alias *)(void *)Stuples[i].payPrimary;
        *dst = ct_select_u32(*dst, src, m);
      } else {
        const uint32_t src =
            *(const u32_may_alias *)(const void *)Stuples[i].paySelf;
        u32_may_alias *dst =
            (u32_may_alias *)(void *)Rtuples[hit - 1].payPrimary;
        *dst = ct_select_u32(*dst, src, m);
      }
    }
  }
...
static int64_t bucket_chaining_join_fixed(const struct table_t *const R,
  ...
  if (!isSPrimary) {
    // R is the primary-key relation: copy R.paySelf -> S.payPrimary on match.
    for (uint32_t b = 0; b < ubins; ++b) {
      ...
      for (uint32_t si = 0; si < U_S; ++si) {
        ...
        // row_t is 32 bytes = 8 u32 words.
        // Layout (u32 words): [key,cntSelf,hashKey,idx,shuffledIdx,paySelf,payPrimary,pad]
        typedef int i32_may_alias __attribute__((may_alias));
        const i32_may_alias *baseWordsAlias =
            (const i32_may_alias *)(const void *)Rmut;
        const int *baseWords = (const int *)baseWordsAlias;

        __m256i payAcc = vZero;
        __m256i matchAcc = vZero;
        ...
        for (; ri + 8u <= U_R; ri += 8u) {
          ...
          const __m256i vSelfWord =
              _mm256_add_epi32(vBaseWord, _mm256_set1_epi32(1));
          const __m256i vPayWord =
              _mm256_add_epi32(vBaseWord, _mm256_set1_epi32(5));
          ...
          const __m256i vRPaySelf =
              _mm256_i32gather_epi32(baseWords, vPayWord, 4);
          ...
          matchAcc = _mm256_or_si256(matchAcc, vMatch);
          payAcc = _mm256_or_si256(payAcc, _mm256_and_si256(vMatch, vRPaySelf));
        }

        hitMask |= horizontal_or_u32x8(matchAcc);
        hitPay |= horizontal_or_u32x8(payAcc);

        for (; ri < U_R; ++ri) {
          const uint32_t ridx = idxR[baseR + ri];
          struct row_t *rrow = &Rmut[ridx];

          const uint32_t match32 = (uint32_t)-(rrow->cntSelf != 0u) &
                                   (uint32_t)-(sCntSelf != 0u) &
                                   (uint32_t)-(rrow->key == sKey);
          const uint32_t rPay =
              *(const u32_may_alias *)(const void *)rrow->paySelf;
          hitMask |= match32;
          hitPay |= (match32 & rPay);
        }
#else
        ...
        u32_may_alias *dst = (u32_may_alias *)(void *)srow->payPrimary;
        *dst = ct_select_u32(*dst, hitPay, hitMask);
      }
    }
  } else {
    // S is the primary-key relation: copy S.paySelf -> R.payPrimary on match.
    for (uint32_t b = 0; b < ubins; ++b) {
      const uint32_t baseR = b * U_R;
      const uint32_t baseS = b * U_S;
      ...
      for (uint32_t si = 0; si < U_S; ++si) {
        ...
        const uint32_t sCntSelf = srow->cntSelf;
        const uint32_t sPay =
            *(const u32_may_alias *)(const void *)srow->paySelf;
        ...
        for (; ri + 8u <= U_R; ri += 8u) {
          ...
          uint32_t matchWords[8];
          _mm256_storeu_si256((__m256i *)matchWords, vMatch);
          for (uint32_t lane = 0; lane < 8u; ++lane) {
            const uint32_t ridx = idxPtr[lane];
            u32_may_alias *dst =
                (u32_may_alias *)(void *)Rmut[ridx].payPrimary;
            *dst = ct_select_u32(*dst, sPay, matchWords[lane]);
          }
        }
        ...
        for (; ri < U_R; ++ri) {
          ...
          const uint32_t match32 = (uint32_t)-(rrow->cntSelf != 0u) &
                                   (uint32_t)-(sCntSelf != 0u) &
                                   (uint32_t)-(rrow->key == sKey);
          u32_may_alias *dst = (u32_may_alias *)(void *)rrow->payPrimary;
          *dst = ct_select_u32(*dst, sPay, match32);
        }
#else
        for (uint32_t ri = 0; ri < U_R; ++ri) {
          const uint32_t ridx = idxR[baseR + ri];
  ...
  if (fixedParts) {
    ...
    for (uint32_t low = (uint32_t)my_tid; low < fanOut1;
      ...
      for (uint32_t high = 0; high < highParts; ++high) {
        ...
        results += args->join_function(&relRt, &relSt, &tmpRt, &output,
                                       args->isSPrimary, args->bins);
        ...
        for (uint64_t i = 0; i < relRt.num_tuples; i++) {
          ...
          *(u32_may_alias *)(void *)args->origRelR[orig_idx].payPrimary =
              *(const u32_may_alias *)(const void *)relRt.tuples[i].payPrimary;
        }
        for (uint64_t i = 0; i < relSt.num_tuples; i++) {
          ...
          *(u32_may_alias *)(void *)args->origRelS[orig_idx].payPrimary =
              *(const u32_may_alias *)(const void *)relSt.tuples[i].payPrimary;
        }
    ...
    while ((task = task_queue_get_atomic(join_queue))) {
      ...
      for (uint32_t i = 0; i < task->relR.num_tuples; i++) {
        ...
        *(u32_may_alias *)(void *)args->origRelR[orig_idx].payPrimary =
            *(const u32_may_alias *)(const void *)task->relR.tuples[i].payPrimary;
      }
      for (uint32_t i = 0; i < task->relS.num_tuples; i++) {
        ...
        *(u32_may_alias *)(void *)args->origRelS[orig_idx].payPrimary =
            *(const u32_may_alias *)(const void *)task->relS.tuples[i].payPrimary;
      }
      args->parts_processed++;
    }
  }
