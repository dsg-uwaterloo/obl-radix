...
// run length (including the per-bucket header row) to shuffledIdx at the end of
// each run. Other positions have shuffledIdx==0. This mirrors the structure of
// parallelCounts() but uses bucketId instead of key and uses shuffledIdx as a
  ...
  // Pass 1: local run-lengths per slice.
  {
    ...
    for (std::size_t t = 0; t < P; ++t) {
      pool.emplace_back([&, t] {
        ...
        // Initialize all shuffledIdx within the slice to 0; then only run-ends
        // will receive a non-zero count.
        rows[sl.begin].shuffledIdx = 0u;
        ...
        for (; i < end; ++i) {
          ...
          rows[i - 1u].shuffledIdx = (~match) & r;
          r = (match & (r + 1u)) | (~match & 1u);
          prev = (match & prev) | (~match & curr);
          rows[i].shuffledIdx = 0u;
        }
        rows[end - 1u].shuffledIdx = r;
        lastLen[t] = r;
      });
    }
    for (auto &th : pool)
  ...
  // Pass 2: merge values across slice boundaries.
  {
    ...
    for (std::size_t t = 1; t < P; ++t) {
      pool.emplace_back([&, t] {
        ...
        rows[idx].shuffledIdx = (~match) & rows[idx].shuffledIdx;
      });
    }
    for (auto &th : pool)
      th.join();
    ...
    for (std::size_t t = 1; t < P; ++t) {
      pool.emplace_back([&, t] {
        ...
        for (std::uint32_t j = sl.begin; j < sl.end; ++j) {
          const std::uint32_t isNZ =
              0u - bool_to_u32(rows[j].shuffledIdx != 0u);
          const std::uint32_t doAdd = (~done) & isNZ;
          rows[j].shuffledIdx += doAdd & mv;
          done |= doAdd;
        }
      });
    }
  ...
  void pad(table_t &tbl) {
    ...
    parallel_slices(M, numThreads_, [&](const Slice &sl) {
      ...
      for (std::uint32_t i = sl.begin; i < sl.end; ++i) {
        ...
        const std::uint32_t runLen = work_[i].shuffledIdx;
        const std::uint32_t realCount = runLen - 1u;
        work_[i].shuffledIdx =
            ct_select_u32(work_[i].shuffledIdx, realCount, isHeader);
      }
    });
    ...
    parallel_slices(p_, numThreads_, [&](const Slice &sl) {
      for (std::uint32_t i = sl.begin; i < sl.end; ++i) {
        const std::uint32_t real = headers_[i].shuffledIdx;
        headers_[i].shuffledIdx = U - real;
      }
    });
    ...
    const std::uint64_t computedNeed64 =
        prefixSumExpandParallel(hdrTbl, slicesHdr, selected_.get(),
                                PrefixSumExpandMode::CountsShuffledIdx);
    if (computedNeed64 != need64) {
      std::uint64_t sumDef = 0;
      std::uint64_t sumReal = 0;
      std::vector<std::uint64_t> bucketReal(p_, 0);
      for (std::uint32_t i = 0; i < M; ++i) {
        if (work_[i].idx == std::numeric_limits<std::uint32_t>::max())
          continue;
        bucketReal[work_[i].hashKey & lowMask_]++;
      }
      for (std::uint32_t i = 0; i < p_; ++i) {
        const std::uint32_t def = headers_[i].shuffledIdx;
        sumDef += def;
        sumReal += (U >= def) ? (U - def) : 0u;
      }
      std::fprintf(stderr,
                   "padTableSort: prefix sum mismatch (N=%u p=%u U=%u "
                   "need=%" PRIu64 " computed=%" PRIu64 " sumDef=%" PRIu64
                   " sumReal=%" PRIu64 " bucketReal=[",
                   N, p_, U, need64, computedNeed64, sumDef, sumReal);
      for (std::uint32_t i = 0; i < p_; ++i) {
        std::fprintf(stderr, "%s%" PRIu64, (i ? "," : ""), bucketReal[i]);
      }
      std::fprintf(stderr, "])\n");
    ...
    parallel_slices(p_, numThreads_, [&](const Slice &sl) {
      for (std::uint32_t i = sl.begin; i < sl.end; ++i) {
        row_t m = headers_[i];
        const std::uint32_t needHere = headers_[i].shuffledIdx;
        const std::uint32_t keep = bool_to_u32(needHere != 0u);
        m.cntSelf = ct_select_u32(0u, 1u, keep);
        m.hashKey &= lowMask_;
