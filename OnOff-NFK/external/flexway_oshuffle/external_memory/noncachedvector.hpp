...
struct Vector {
  ...
  // writer always overwrites data between begin and end
    ...
    void flush() {
	  ...
	  // Compatibility shim: newer flex-way implementations also define a deferred
	  // writer type. For oshuffle we don't need special buffering semantics, so
	  // this simply reuses Writer.
	  struct DeferedWriter : Writer {
	    DeferedWriter() = default;
