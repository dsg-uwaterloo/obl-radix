...
int64_t bucket_chaining_join(const struct table_t *const R,
  ...
  for (uint32_t i = 0; i < numS; i++) {
    ...
    for (int hit = bucket[idx]; hit > 0; hit = next[hit - 1]) {
      ...
      uint64_t match = -(Stuples[i].key == Rtuples[hit - 1].key);
      const char *src_pay = (const char *)&Stuples[i].paySelf;
      char *dst_pay = (char *)&Rtuples[hit - 1].payPrimary;

      // printf("  S.key = %lu, R.key = %lu\n", (uint64_t)Stuples[i].key,
      //        (uint64_t)Rtuples[hit - 1].key);

      // printf("  match = %lu\n", match);

      // Assuming 8-byte (uint64_t) payloads:
      // printf("  src_pay = %lu\n", *((uint64_t *)src_pay));
      // printf("  dst_pay(before) = %lu\n", *((uint64_t *)dst_pay));

      *((uint64_t *)dst_pay) =
          (match & *((uint64_t *)src_pay)) | (~match & *((uint64_t *)dst_pay));

      // printf("  dst_pay(after) = %lu\n\n", *((uint64_t *)dst_pay));
      // }
    }
  }
