  ...
  for (uint32_t t = 0; t < P; ++t) {
    pool.emplace_back([&, t] {
      ...
      for (; i + 4 <= end; i += 4) {
        ...
        table.tuples[i].hashKey = triple32((table.tuples[i].key & ~m0) | (d0 & m0));
        table.tuples[i + 1].hashKey = triple32((table.tuples[i + 1].key & ~m1) | (d1 & m1));
        table.tuples[i + 2].hashKey = triple32((table.tuples[i + 2].key & ~m2) | (d2 & m2));
        table.tuples[i + 3].hashKey = triple32((table.tuples[i + 3].key & ~m3) | (d3 & m3));
      }
      ...
      for (; i < end; ++i) {
        ...
        table.tuples[i].hashKey = triple32((table.tuples[i].key & ~mask) | (dummy & mask));
      }
    });
  }
  for (auto &th : pool)
