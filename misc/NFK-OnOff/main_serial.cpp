...
static void printOutput(const char *label, const table_t &tbl) {
  for (uint64_t i = 0; i < tbl.num_tuples; ++i) {
    ...
    printf("key=%u cntSelf=%u cntExpand=%u idx=%u shuffledIdx=%u hashKey=%u payload=%s\n", rec.key,
           rec.cntSelf, rec.cntExpand, rec.idx, rec.shuffledIdx, rec.hashKey, rec.pay);
  }
}
